...
async function handler(ctx) {
    try {
        ...
        // Place the bet
        await autoBot.placeBullBet(currentEpoch, amount);
        
        await ctx.reply(`✅ Manual BULL bet placed for epoch ${currentEpoch}\nAmount: ${ethers.formatEther(amount)} BNB`);
        
    } catch (error) {
        console.error('Error in betbull command:', error);
        await ctx.reply(`❌ Error placing bull bet: ${error.message}`);
    }
}
